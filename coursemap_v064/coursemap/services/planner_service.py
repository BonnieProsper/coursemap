...
class PlannerService:
    ...
    def generate_filled_plan(
        ...
        # Identify filler candidates: same subject-prefix courses not already
        # in the base plan, active in the requested campus/mode, at an
        # appropriate level.
        ...
        excluded_all  = planned_codes | prior_codes | excluded_courses
        ...
        for code, course in self.courses.items():
            ...
            if not any(o.campus == campus and o.mode == mode for o in course.offerings):
                continue
            if course.level > 300 or course.level > max_level + 100:
                continue
            pfx = code[:3]
            if pfx in prefix_rank:
                prefer = 0 if code in preferred_electives else 1
                candidates.append((prefer, prefix_rank[pfx], course.level, code))

        candidates.sort()

        filler_codes: list[str] = []
        running = 0
        for _, _, _, code in candidates:
            if running >= effective_gap:
                break
            cr = self.courses[code].credits
            if running + cr > effective_gap + 30:
                continue
