...
class PlanRequest(BaseModel):
    ...
    no_summer: bool = Field(False, description="Skip Summer School semesters.")
...
def _plan_to_out(
    ...
    meta: dict[str, Any] = {
        ...
        "free_elective_gap": max(0, gap - plan.transfer_credits),
...
def list_majors(
    ...
    limit:  int        = Query(50,   ge=1, le=380, description="Max results."),
):
    svc = _svc()
    majors = svc.majors
...
def generate_plan_ical(request: Request, req: PlanRequest):
    ...
    filename = f"degree_plan_{req.major.replace(' ', '_')[:40]}.ics"
    return Response(
        content=ics_content,
