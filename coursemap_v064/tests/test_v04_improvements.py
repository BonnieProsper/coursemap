...
def test_ui_has_validation_panel_markup():
    r = client.get('/')
    assert 'validation-panel' in r.text
    assert 'renderValidationPanel' in r.text

def test_ui_has_year_grouping_markup():
    r = client.get('/')
    assert 'year-group-header' in r.text
    assert 'groupSemestersByYear' in r.text

def test_ui_has_mark_completed_markup():
    r = client.get('/')
    assert 'markCompleted' in r.text
    assert 'markSemesterComplete' in r.text

def test_ui_has_progress_bar_markup():
    r = client.get('/')
    assert 'degree-progress-wrap' in r.text
