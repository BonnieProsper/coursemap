"""
UI configuration, start-semester handling, and scheduling option plumbing.

Covers:
- start_semester wired end-to-end (generator → search → service → API)
- dynamic start_year default (current year, not hardcoded)
...
def test_ui_has_start_semester_selector():
    ...
    # New UI uses id="start-sem" with S1/S2/SS options
    assert 'id="start-sem"' in html or 'id="start-semester"' in html
