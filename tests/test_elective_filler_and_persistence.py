"""
ElectiveFiller, plan persistence, and streaming/validation endpoints.
...
class TestPlanStore:
    def test_connection_is_closed_after_use(self):
        """
        Regression test for a real bug: `with sqlite3.connect(...) as conn:`
        only manages the transaction (commit/rollback on exit), it does NOT
        close the connection, unlike almost every other Python resource's
        context manager. Every _PlanStore method used to rely on exactly
        that pattern directly, leaking one open connection per call for the
        life of the process. Invisible on Linux (an unlinked-but-open file
        still gets deleted), but on Windows an open handle blocks deleting
        or renaming the file at all, this is what caused
        `PermissionError: The process cannot access the file` when tests
        tried to clean up their temp directories. Confirms the connection
        returned by the internal context manager is actually closed by
        trying to use it afterward and expecting sqlite3 to reject it.
        """
